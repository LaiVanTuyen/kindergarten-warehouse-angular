    ...
    >
      <form [formGroup]="form" (ngSubmit)="submit()" class="space-y-4">
        ...
        <app-form-field label="Mô tả" [control]="form.controls.description">
          <textarea formControlName="description" rows="2" maxlength="300" class="w-full px-3 py-2 border border-line rounded-lg text-sm focus:outline-none focus-visible:ring-2 focus-visible:ring-focus focus:border-transparent resize-none"></textarea>
        </app-form-field>
          ...
          <div class="grid grid-cols-6 gap-2">
            @for (emoji of emojis; track emoji) {
              <button
                ...
                class="w-10 h-10 rounded-lg border-2 flex items-center justify-center text-xl transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-focus"
        ...
        <div class="grid grid-cols-2 gap-3">
          <app-form-field label="Nền tảng">
            <select formControlName="platform" class="w-full px-3 py-2 border border-line rounded-lg text-sm bg-surface focus:outline-none focus-visible:ring-2 focus-visible:ring-focus">
...
export class CategoryFormDialogComponent {
  ...
  readonly form = this.fb.nonNullable.group({
    ...
    isPublic: [this.data.category ? this.data.category.visibility !== 'PRIVATE' : true],
